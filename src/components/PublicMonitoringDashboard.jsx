...
function PublicMonitoringDashboard({ onLogin }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setErrorMessage(
        error.message ||
          "Data monitoring belum dapat dimuat.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#FFF1F2] via-[#FDF2F8] to-[#F8FAFC]">
      <header className="border-b border-[#8F1024]/15 bg-white/90 shadow-sm backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-6">
          ...
          <button
            type="button"
            onClick={onLogin}
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#C5163A] via-[#8F1024] to-[#5B000A] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:brightness-95"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
...
export default PublicMonitoringDashboard;